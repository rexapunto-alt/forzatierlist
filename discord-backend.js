...
client.once('ready', () => {
    ...
    console.log(`📢 Bildirim kanalı ID: ${CHANNEL_ID}`);
});
...
// Bot'u başlat
if (BOT_TOKEN && BOT_TOKEN !== 'BURAYA_DISCORD_BOT_TOKEN_YAZINIZ') {
    client.login(BOT_TOKEN).catch(err => {
        console.error('❌ Discord bot giriş başarısız:', err.message);
        console.log('⚠️ Lütfen config.json dosyasındaki botToken değerini güncelleyin!');
    });
} else {
    console.log('⚠️ Discord bot token ayarlanmamış!');
    console.log('📝 Lütfen config.json dosyasında botToken değerini güncelleyin.');
}
...
app.listen(DISCORD_PORT, () => {
    ...
    console.log(`🔗 Webhook URL: http://localhost:${DISCORD_PORT}/discord-notify`);
});
